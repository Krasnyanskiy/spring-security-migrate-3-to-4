...
[source,java]
----
public CustomWebAuthenticationDetailsSource implements AuthenticationDetailsSource<HttpServletRequest, WebAuthenticationDetails> {
...
for(SecurityFilterChain entry : mappings) {
	RequestMatcher matcher = entry.getMatcher();
...
If you are looking up the `WebSecurityExpressionHandler` with the ApplicationContext, like the following:
...
ResolvableType resolvableType =
	ResolvableType.forClassWithGenerics(SecurityExpressionHandler.class, FilterInvocation.class);
SecurityExpressionHandler<FilterInvocation> handler =
	(SecurityExpressionHandler<FilterInvocation>) context.getBean(resolvableType.resolve());
----
